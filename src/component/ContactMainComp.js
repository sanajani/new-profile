      ...
      <div className=" grid  md:grid-cols-2 md:gap-20 mt-20">
        ...
        <div className="">
          <form className="flex flex-col">
            ...
            <input value={inputValue.name} name="name" onChange={getInputFieldsValue} className="mx-10 md:mx-8 rounded-md bg-[#1f1f1f] text-gray-300 outline-none border-none text-xl md:text-2xl py-4 pl-4 my-2" type="text" id="name" />
            {/* </div> */}
            {/* <div> */}
            <label className=" mx-10 md:mx-8 text-base md:text-xl mt-3" htmlFor="email">Your Email</label>
            <input value={inputValue.email} name="email" onChange={getInputFieldsValue} className="mx-10 md:mx-8 rounded-md bg-[#1f1f1f] text-gray-300 outline-none border-none text-xl md:text-2xl py-4 pl-4 my-2" type="email" id="email" />
            {/* </div> */}
            {/* <div> */}
            <label className=" mx-10 md:mx-8 text-base md:text-xl mt-3" htmlFor="textArea">Your Message</label>
            <textarea value={inputValue.message} onChange={getInputFieldsValue} className="mx-10 md:mx-8 rounded-md bg-[#1f1f1f] resize-none min-h-[200px] text-gray-300 outline-none border-none text-xl md:text-2xl py-4 pl-4 my-2" name="message" id="textArea" cols="" rows=""></textarea>
            {/* </div> */}
            <div className="mt-5 mx-10 md:mx-8">
              <button onClick={submitForm} className="bg-[#1f1f1f] py-3 px-12 rounded-md text-xl md:text-2xl">Send</button>
            </div>
          </form>
        </div>
      </div>
